/* ============================================================
   캘린더 자동 수집 크론 — 매일 1회 (vercel.json crons)
   ...
   보호: CRON_SECRET 설정 시 Authorization: Bearer <secret> 요구
        (Vercel Cron이 자동으로 이 헤더를 붙여 호출한다)
   ============================================================ */
...
module.exports = async (req, res) => {
  ...
  if (!sb.ready()) {
    return res.status(200).json({ skipped: true, reason: 'SUPABASE_SERVICE_ROLE_KEY 미설정 — 수집을 건너뜀' });
  }
  ...
  res.status(200).json({ ok: true, synced: report });
};
