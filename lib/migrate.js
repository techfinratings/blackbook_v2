...
async function runMigration(force) {
  if (!sb.ready()) throw new Error('SUPABASE_SERVICE_ROLE_KEY 미설정');
  const report = {};
  try {
    // 자료실
    ...
    return { ok: true, migrated: report };
  } catch (e) {
    console.error('migrate error:', e.message);
    return { ok: false, error: e.message, partial: report };
  }
}
