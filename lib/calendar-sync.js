...
function statutory(year, month) {
  ...
  if ([1, 7].includes(month)) push(25, '부가가치세 확정신고·납부', 'tax');
  if ([4, 10].includes(month)) push(25, '부가가치세 예정신고·예정고지 납부', 'tax');
...
async function sync() {
  ...
  const flagged = await flagGuides(allLaws).catch(e => { console.error('guide flag error:', e.message); return 0; });
  return { ok: true, synced: report, guideFlags: flagged };
}

/* ── 실무백과 자동 감지 — 개정 법령이 백과 주제와 겹치면 '검토 필요' 플래그 ──
   관리자(/admin 백과 탭)에서 내용 확인·수정 후 '확인 처리'로 플래그를 지운다. */
const GUIDE_WATCH = [
  { slug: 'withholding',   title: '원천징수(원천세) 신고 안내', keywords: ['원천징수', '소득세법 시행령', '소득세법 시행규칙'] },
  { slug: 'vat',           title: '부가가치세 신고 안내',       keywords: ['부가가치세'] },
  { slug: 'corporate',     title: '법인세 신고 안내',           keywords: ['법인세'] },
  { slug: 'global-income', title: '종합소득세 신고 안내',       keywords: ['소득세법', '종합소득'] },
  { slug: 'year-end',      title: '연말정산 안내',              keywords: ['연말정산', '소득세법 시행령'] },
];
async function flagGuides(laws) {
  if (!laws.length) return 0;
  const flags = [];
  for (const law of laws) {
    for (const g of GUIDE_WATCH) {
      if (g.keywords.some(k => (law.title || '').includes(k))) {
        flags.push({
          guide_slug: g.slug, guide_title: g.title,
          law_title: (law.title || '').slice(0, 300), law_link: law.link || '',
          eff_date: `${law.year}${pad(law.month)}${pad(law.day)}`,
        });
      }
    }
  }
  if (flags.length) await sb.upsert('guide_flags', flags, 'guide_slug,law_title,eff_date');
  return flags.length;
}
