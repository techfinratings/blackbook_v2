/* ============================================================
   Google Sheets 서비스계정 헬퍼 — [이관 완료 후 비활성 폴백]
   Supabase 이관이 끝나 기본적으로 사용되지 않는다. 비밀키는 코드에
   두지 않으며, 재이관 등 임시로 필요하면 Vercel 환경변수로 주입:
     GOOGLE_SA_EMAIL / GOOGLE_SA_KEY / LEADS_SHEET_ID
   키 미설정 시 모든 호출은 명확한 오류를 던지고, 호출부(catch)가
   처리한다. 다음 정리 단계에서 파일 자체를 제거할 예정.
   ============================================================ */
...
function assertKey() {
  if (!PRIVATE_KEY) throw new Error('구글시트 연동은 이관 완료로 비활성화되었습니다 (GOOGLE_SA_KEY 미설정).');
}
