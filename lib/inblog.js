...
// onePage=true면 첫 페이지 한 번만 조회(홈처럼 상위 몇 건만 필요할 때 응답 단축)
async function fetchPublished(key, perPage, onePage) {
  const arr = onePage
    ? ((await fetchList(key, perPage, 1)) || {}).data || []
    : await fetchAll(key, perPage);
  return arr
...
// 제목·키워드 기반 카테고리 분류 (콘텐츠 필터용 — 세무/회계/재무/법령/경영/정책자금)
function classifyCat(title, keywords) {
  const t = String(title || '') + ' ' + String(keywords || '');
  if (/정책자금|보조금|지원사업|정부지원|융자|보증(기금)?/.test(t)) return '정책자금';
  if (/부가가치세|법인세|소득세|원천|연말정산|세무|세금|절세|증여|상속|가업승계|과세|국세/.test(t)) return '세무';
  if (/회계|결산|외부감사|재무제표|리스\s?회계|IFRS|K-IFRS|분개|감가상각|공시/.test(t)) return '회계';
  if (/중대재해|근로|노동|법령|규제|컴플라이언스|개정법|시행령/.test(t)) return '법령';
  if (/경영|전략|조직|리더|CEO|CFO|런웨이|스타트업|의사결정/.test(t)) return '경영';
  return '재무';
}

module.exports = { BLOG_ID, isPublished, fetchList, fetchAll, fetchPublished, extractSeo, rawSample, classifyCat };
